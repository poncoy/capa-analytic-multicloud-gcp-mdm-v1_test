...
direccion as (
    ...
    FROM
        `{PROJECT_ID}.lan_mdm_ingesta.CON__datos_contacto` dc
        CROSS JOIN dc.detalle_datos_contacto ddc
    WHERE
        ddc.des_tipo = 'COMERCIAL'
        and dc.periodo in (
            select
                periodo
            from
...
cmp as (
    ...
    FROM
        `{PROJECT_ID}.lan_mdm_ingesta.PER__prospecto_persona_merge` pp
    WHERE
        pp.ind_royal != 'SI'
        and pp.num_edad >= 18
        and pp.periodo in (
...
FROM
    cmp
    LEFT JOIN direccion dir ON cmp.id_persona = dir.id_persona
WHERE
