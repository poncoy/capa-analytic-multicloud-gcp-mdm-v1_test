...
Fuentes de datos: Capa analytics anl_siniestro.siniestro_detalle_salud
                  Capa analytics anl_siniestro.carta_garantia_solicitud
Destino: Capa rapida Bigtable uni__siniestro_carta_garantia_salud
...
procedimiento AS (
  ...
  WHERE periodo = DATE(TIMESTAMP_TRUNC(CURRENT_TIMESTAMP(), MONTH, "America/Lima"))
),
base_carta AS (
  WITH version AS (
    SELECT
      id_siniestro AS id_siniestro_carta,
      num_carta_garantia,
      DATE(version.fec_solicitud) AS fec_solicitud,
      version.num_solicitud AS num_solicitud_carta,
      version.mnt_solicitado,
      version.por_cobertura_carta,
      version.des_est_solicitud_origen AS des_est_solicitud_origen_carta,
      version.des_tipo_procedimiento AS des_tipo_procedimiento_carta,
      version.des_procedimiento AS des_procedimiento_carta,
      version.num_poliza AS nro_poliza_carta,
      version.mnt_autorizado,
    FROM `rs-nprd-dlk-dt-anlyt-meve-2034.anl_salud.carta_garantia_solicitud` AS solicitud
    LEFT JOIN UNNEST(carta_garantia_version) version
    WHERE solicitud.periodo = DATE_TRUNC(CURRENT_DATE(), MONTH)
      AND version.des_est_solicitud_origen <> 'ANULADO'
  )
  SELECT
    carta.num_carta_garantia,
    CASE
      WHEN carta.des_est_carta_garantia_origen IN ('REGISTRADA \\ GENERADA','APROBADA\\ PDTE.DE LIQUIDACION(FAXEADA)') THEN 'PENDIENTE'
      WHEN carta.des_est_carta_garantia_origen = 'PROCESADA (CON LIQUIDACION)' THEN 'LIQUIDACION'
      ELSE carta.des_est_carta_garantia_origen
    END AS est_carta_garantia,
    carta.des_est_carta_garantia_origen,
    carta.num_documento_contratante,
    carta.agrupacion_cobertura AS agrupacion_cobertura_carta,
    carta.des_grupo_renovacion_salud,
    carta.nom_completo_contratante,
    carta.des_razon_social_proveedor,
    carta.des_diagnostico,
    carta.tip_documento_afiliado AS tip_documento_afiliado_carta,
    carta.num_documento_afiliado AS num_documento_afiliado_carta,
    carta.nom_completo_afiliado AS nom_completo_afiliado_carta,
    carta.nom_completo_titular AS nom_completo_titular_carta,
    carta.des_producto AS des_producto_carta,
    version.id_siniestro_carta,
    version.fec_solicitud,
    version.num_solicitud_carta,
    version.mnt_solicitado,
    version.por_cobertura_carta,
    version.des_est_solicitud_origen_carta,
    version.des_procedimiento_carta,
    version.nro_poliza_carta,
    version.mnt_autorizado
  FROM `rs-nprd-dlk-dt-anlyt-meve-2034.anl_salud.carta_garantia_solicitud` AS carta
  LEFT JOIN version ON carta.num_carta_garantia = version.num_carta_garantia
...
base_siniestro AS (
  ...
  WHERE salud.periodo = DATE(TIMESTAMP_TRUNC(CURRENT_TIMESTAMP(), MONTH, "America/Lima"))
),
reporte_rar AS (
  SELECT
    bs.*,
  ...
  FULL JOIN base_carta c ON c.id_siniestro_carta = bs.id_siniestro
),
without_row_key AS (
  SELECT
    ...
    nom_medico,
    num_colegio_medico,
    des_razon_social_proveedor,
    num_siniestro,
    cod_proceso_bpm,
    num_carta_garantia,
    des_est_carta_garantia_origen ,
    fec_ult_liquidacion,
    periodo
  FROM reporte_rar
  WHERE 1 = 1
  ...
  ORDER BY por_cobertura_carta DESC
)
...
SELECT DISTINCT
    CONCAT(
        ...
        IFNULL(CAST(fec_solicitud AS STRING), 'NULL'), '#',
        IFNULL(CAST(des_producto AS STRING), 'NULL'), '#',
        IFNULL(CAST(agrupacion_cobertura AS STRING), 'NULL'), '#',
        IFNULL(CAST(mnt_solicitado AS STRING), 'NULL'), '#',
        IFNULL(CAST(mnt_autorizado AS STRING), 'NULL'), '#',
        IFNULL(CAST(por_cobertura_carta AS STRING), 'NULL'), '#',
        ...
        IFNULL(CAST(des_razon_social_proveedor AS STRING), 'NULL'), '#',
        IFNULL(CAST(num_siniestro AS STRING), 'NULL'), '#',
        IFNULL(CAST(cod_proceso_bpm AS STRING), 'NULL'), '#',
        IFNULL(CAST(num_carta_garantia AS STRING), 'NULL'), '#',
        IFNULL(CAST(des_est_carta_garantia_origen AS STRING), 'NULL'), '#',
