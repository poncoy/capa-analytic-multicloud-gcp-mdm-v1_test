...
Destino: Capa rapida Bigtable uni__siniestro_procedimiento_salud
...
DECLARE periodo_siniestro DATE;

SET periodo_siniestro = DATE_TRUNC(CURRENT_DATE('America/Lima'), MONTH);
...
procedimiento AS (
  ...
  WHERE periodo = periodo_siniestro
),
base_siniestro AS (
  SELECT
    salud.id_siniestro,
  ...
  WHERE salud.periodo = periodo_siniestro
),
reporte_rar AS (
  SELECT
    bs.*,
...
without_row_key AS (
  SELECT
    ...
    ROUND(SUM(mnt_gasto_presentado_tec_sol), 2) AS mnt_gasto_presentado_sol,
    tip_procedencia,
    nom_completo_afiliado,
    periodo
  FROM reporte_rar
  WHERE 1 = 1
    AND des_procedimiento IS NOT NULL
    AND (
...
SELECT DISTINCT
    CONCAT(
        ...
        IFNULL(CAST(mnt_gasto_presentado_sol AS STRING), 'NULL'), '#',
