...
with siniestro_detalle_ult_periodo as (
    select max(periodo) as periodo/********************** CONTROL DE CAMBIOS *****************************
Organizacion: RIMAC
Programa: Portal Salud Siniestro
Creado por:
Fecha de Creación: 03/10/2023
...
with siniestro_detalle_ult_periodo as (
    ...
    from 
    `rs-nprd-dlk-dt-anlyt-meve-2034.anl_salud.siniestro_detalle_salud_bk`    
    WHERE
...
base AS(
  SELECT  
    ...
    CAST(IFNULL(DATE(fec_devolucion_siniestro),CURRENT_DATE()) AS STRING) as C006_fec_devolucion,
...
from (
    select  
          ...
          periodo 
       from `rs-nprd-dlk-dt-anlyt-meve-2034.anl_salud.siniestro_detalle_salud_bk` sds
       left join unnest (atencion_salud) ats
       left join unnest (grupo_devolucion) gd
    where
    1=1 
    and sds.periodo in (select periodo from siniestro_detalle_ult_periodo)
...
group by 1,2,3,4,5,6,7,9,12,13)
SELECT CONCAT(b.C006_fec_notificacion,'#',b.C006_fec_devolucion,'#',b.C006_num_documento_proveedor,'#',b.C006_id_estado_siniestro_origen,'#',b.C006_ind_registrado_trama,'#',b.C006_des_origen_factura,'#', b.C006_des_grupo_motivo_devolucion,'#',b.C006_cod_sede_proveedor_siniestro) as C006_row_key, b.* FROM base b
    ...
    from 
    `rs-nprd-dlk-dt-anlyt-meve-2034.anl_salud.siniestro_detalle_salud`    
    WHERE
    periodo > DATE_ADD(CURRENT_DATE(), INTERVAL -2 MONTH)
),
base AS(
...
from (
    select  
          ...
          case when gd.cod_grupo_motivo_devolucion = 'B' or gd.cod_grupo_motivo_devolucion = 'C' then 2 -- MEDICO 
              ...
              as peso_devolucion,
          periodo 
       from `rs-nprd-dlk-dt-anlyt-meve-2034.anl_salud.siniestro_detalle_salud` sds
       left join unnest (atencion_salud) ats
       left join unnest (grupo_devolucion) gd
    where
    1=1 
...
SELECT CONCAT(b.C006_fec_notificacion,'#',b.C006_num_documento_proveedor,'#',b.C006_id_estado_siniestro_origen,'#',b.C006_ind_registrado_trama,'#',b.C006_des_origen_factura,'#', b.C006_des_grupo_motivo_devolucion,'#',b.C006_cod_sede_proveedor_siniestro) as C006_row_key, b.* FROM base b
